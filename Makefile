...
# the three kernels, boot configuration and the game's default settings. The
# WAD is not here and is not ours to ship — see the README for what to put in
# doom/ and where it legitimately comes from.
CARD_DIR = build/sd-card
card: kernels
	@rm -rf $(CARD_DIR)
	@mkdir -p $(CARD_DIR)/doom
	...
	@cp host/default.cfg host/chocolate-doom.cfg $(CARD_DIR)/doom/
	@echo "  STAGED $(CARD_DIR)/"
	@echo "  The Raspberry Pi firmware files and the WAD are not staged here."
	@echo "  Put an IWAD in $(CARD_DIR)/doom/ before writing the card."
